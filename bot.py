...
def city_ask(message: Message):  # Выбор города
    ...
    button_swamp = InlineKeyboardButton(text='Боло.. Ой, СПб', callback_data="set_city_swamp")
    ...
    if city == 'moscow':
        ...
        if '18' > hour > '08':
            if activity == 'flex':
                if day in weekend:
                    bot.send_message(chat_id=message.chat.id,
                                     text='Лучший флекс в выходные? WRAVE, естественно.'
                                          'Сумасшедшие тусовки от профессионалов в '
                                          'популярных клубах с приглашёнными диджеями '
                                          'не оставят вам шанса заскучать. Следите за событиями '
                                          'в оффициальном инстаграмме: '
                                          'https://www.instagram.com/_wrave_/ '
                                          '(На правах рекламы.)')
                else:
                    bot.send_message(chat_id=message.chat.id,
                                     text='Флексить в такое время особо негде...'
                                          'Может, пока почиллим? А там гляди откроются '
                                          'какие-нибудь бары... /activity_set')
            elif activity == 'chill':
                if month in ['Jan', 'Feb', 'Dec']:  # Зимние спокойные развлечения
                    bot.send_message(chat_id=message.chat.id,
                                     text='Традиционное зимнее развлечение '
                                          'в Петербурге - конечно же Шуваловка! '
                                          'Погрузитесь в аутентичную атмосферу '
                                          'древнерусской Масленицы: '
                                          'пройдитесь по ярморочным лавочкам '
                                          'испытайте удачу в конкурсах '
                                          'или попробуйте себя в кузнечном деле! '
                                          'Также в наличии склоны для санок, '
                                          'ватрушек и всего, чего вам вздумается. '
                                          'http://www.shuvalovka.ru/')

                    bot.send_message(chat_id=message.chat.id,
                                     text='Если лень ехать в Шуваловку, '
                                          'всегда можно насладиться великолепием '
                                          'зимнего леса прямо в спальном районе,'
                                          'ведь по размерам Сосновка - скорее лес, '
                                          'чем обычный парк. Зимой здесь особенно красиво,'
                                          'но не забывайте одеться потеплее! '
                                          'bit.ly/Sosnovka')

                    bot.send_message(chat_id=message.chat.id,
                                     text='Как бы холодно не было на улице,'
                                          'всегда можно сходить в кино.'
                                          'Смотрите новинки в кинотеатрах Петербурга: '
                                          'https://kinoteatr.ru/')

                else:
                    bot.send_message(chat_id=message.chat.id,
                                     text='Хоть лично я и предпочитаю гулять в Сосновке зимой,'
                                          'там красиво круглый год. Снегопады сменяются весенними '
                                          'первоцветами или листопадами, а летом даже можно купаться в '
                                          'местном пруду. Насладиться природой здесь можно всегда:'
                                          'bit.ly/Sosnovka')

                    bot.send_message(chat_id=message.chat.id,
                                     text='Если погода сносная, можно посетить Диво-Остров - '
                                          'отличный парк атракционов, где и дети и взрослые найдут '
                                          'себе что-нибудь по вкусу. Для адреналиновых наркоманов от автора'
                                          'лично рекомендуется атракцион Великолукского Мясокомбината - '
                                          'с покерфейсом оттуда вы уж точно не выйдете. '
                                          'https://www.divo-ostrov.ru/')
            else:
                bot.send_message(chat_id=message.chat.id,
                                 text='Пожалуйста, выберите тип активности: /activity_set')

        else:
            if activity == 'flex':
                bot.send_message(chat_id=message.chat.id,
                                 text='Не знаешь где пофлексить - иди в Молодость и зови всех '
                                      'друзей. Одни из лучших, если не лучшие, кальяны в городе и '
                                      'распологающая атмосфера не дадут заскучать. '
                                      'https://vk.com/spbmolodost')
                bot.send_message(chat_id=message.chat.id,
                                 text='Бар - это всегда хорошо, а если бар - это Буковски, то хорошо '
                                      'вдвойне. Крафтовый стаут здесь течёт по усам только так. Огромный лайк '
                                      'лично от автора. http://russiancraftbeer.ru/place/bukowski-bar')

            elif activity == 'chill':
                bot.send_message(chat_id=message.chat.id,
                                 text='Ночной Питер - что может быть романтичнее? Отправляйтесь в центр'
                                      'и прогуляйтесь по набережным, ну или просто пройдите по Невскому: '
                                      'одни только уличные музыканты чего стоят... Личный респект группе Штрудель '
                                      'от автора, ищите их на углу Думской и Невского в долгие питерские вечера. '
                                      'Группа Штрудель: https://vk.com/shtruedel')
